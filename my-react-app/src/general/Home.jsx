...
const Home = () => {
  ...
  useEffect(() => {
    const fetchVideos = async () => {
      try {
        const response = await axios.get('http://localhost:3000/api/food/api/food', {
          withCredentials: true,
        });
        console.log("Fetched data:", response.data); // 👈 Check structure

        // Adjust this according to your backend response
        setVideo(response.data.foodItems);
        console.log(response.data.foodItems,"videos");
      } catch (error) {
  ...
  useEffect(() => {
    axios.get('http://localhost:3000/api/food/api/food', {
      withCredentials: true,
    }).then((response) => { 
      setVideo(response.data.foodItems);
    });
      ...
      <div className="reels-container">
        {video.map((r) => (
          ...
          <section className="reel-item" key={r._id}>
            ...
            {/* <img src={r.video} alt={r.name} className="reel-poster" /> */}
